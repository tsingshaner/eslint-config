...
- 6bb2abc: Remove `react` and `unocss` preset options along with `eslint-plugin-react` and `@unocss/eslint-plugin` support.
...
- 6bb2abc: - Update dependencies and the pinned pnpm package manager version.
  - Add `.DS_Store` to the default ignored files.
...
- 61d473a: - Normalize Changesets `config.json` (key ordering, `ignore` as an array).
  - Extend exported `cspell` preset `ignorePaths` for npm/pnpm/yarn/bun lockfiles.
